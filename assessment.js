...
// repeats strings. Example: 4->12, 2.5->7.5, 'Awesome'->'AwesomeAwesomeAwesome'
	function tripleTheFun(param){
		var ender;
		if(/\d/g.test(param) === false){
			ender = param + param + param;
		} else {
			ender = param * 3;
		}
		return ender;
...
// return the value from the object that corresponds to the property
	...
	function getPropertyValue(object, prop){
...
// the current time as a Date object
	...
	function clockIn(employee, departName){
		var chatMessage = {
			name: employee,
...
function callerBack(holla, string){
		holla(string + " back");
}
